...
class TextDS:
    def __init__(self, path, preprocessor=Preprocessor(), num_terms=10000, batch_size=__settings__.batch_size, split=__settings__.split, window_size=__settings__.window_size, dev='cpu'):
        ...
        """
        self.df = pd.read_csv(path).sample(frac=1.0, random_state=6490)# column of df is Index(['Unnamed: 0', 'text', 'label', 'tokens', 'ids'], dtype='object')
    ...
    def apply_preprocessor(self, preprocessor):
        ...
        self.token_to_count = Counter([x for l in self.df['tokens'] for x in l])# a dict which tokens to corresponding num
        self.vocab = list([term for term, count in self.token_to_count.most_common(self.num_terms)]) # the most common 10000 vocab
        self.token_to_id = {self.vocab[i]: i+2 for i in range(len(self.vocab))}# a dict which stores token to id(id+2)
    ...
    def split(self, train, valid):
        ...
        self.train = slice(0, self.train_index)# slice(start, stop, step)
    ...
    def shuffle(self):
        self.df.iloc[self.train] = self.df.iloc[self.train].sample(frac=1.0) # iloc is like slice, it could slice as a 2 dim Dataflame
    ...
    def batch_method(self, batch):
        ...
        # TODO a list of w*2 word ids from the i'th context window. i'th element of y should be the centre word of the i'th context window.
        x=[]
        y=[]
        for sentence in sentences:
            for i in range(len(sentence)):
                slide_window = sentence[i:i + 2 * w + 1]
                y.append(slide_window.pop(w))
                x.append(slide_window)
...
class LabelledTextDS(TextDS):
    ...
    def get_vector_representation(self):
        ...
        vectorizer = CountVectorizer(lowercase=False,
                                     ...
                                     stop_words=None,
                                     max_features=int(0.1*self.num_terms),  ## This only keeps the 100 most frequent words as features
                                     ngram_range=(1, 2),  ## This uses only unigram counts
                                     binary=True)       ## This sets the features to be frequency counts
